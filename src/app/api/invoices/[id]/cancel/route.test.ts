...
describe('PUT /api/invoices/:id/cancel', () => {
  beforeEach(() => {
    ...
    invoicesBackend.cancelInvoice.mockReset();
  });
  ...
  it('returns 401 when there is no access token', async () => {
    ...
    expect(invoicesBackend.cancelInvoice).not.toHaveBeenCalled();
  });
  ...
  it('cancels the invoice for a SystemAdmin and returns the new status', async () => {
    ...
    invoicesBackend.cancelInvoice.mockResolvedValue({ Id: 'invoice-1', Status: 'Cancelled' });

    const response = await PUT(request, paramsFor('invoice-1'));
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body).toEqual({ id: 'invoice-1', status: 'Cancelled' });
    expect(invoicesBackend.cancelInvoice).toHaveBeenCalledWith('invoice-1', token);
  });
  ...
  it('returns a normalized error when the backend call fails', async () => {
    ...
    invoicesBackend.cancelInvoice.mockRejectedValue(new Error('invoice not found'));
