 ...
 *
 * Transitions a Sent invoice to Paid (`Invoice/MarkInvoicePaid`, no request
 * body) - backs the "Mark as Paid" action on `/invoices/[id]`. Restricted to
 * `PROJECT_MANAGEMENT_ROLES` - see `app/api/invoices/route.ts` for the
 * rationale.
 */
export async function PUT(_request: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  if (!role || !PROJECT_MANAGEMENT_ROLES.includes(role)) {
    return NextResponse.json(
      { message: 'Only a System Admin or Project Admin can mark invoices as paid.' },
      { status: 403 }
    );
  }

  try {
    const dto = await invoicesBackend.markInvoicePaid(id, accessToken);
    ...
    const details = getBackendErrorDetails(error, 'Could not mark the invoice as paid.');
