...
const request = new Request('https://example.com/api/invoices/invoice-1/send', { method: 'PUT' });

describe('PUT /api/invoices/:id/send', () => {
  beforeEach(() => {
    mockCookieStore.get.mockReset();
    invoicesBackend.sendInvoice.mockReset();
  });
  ...
  it('returns 401 when there is no access token', async () => {
    ...
    expect(invoicesBackend.sendInvoice).not.toHaveBeenCalled();
  });
  ...
  it('sends the invoice for a ProjectAdmin and returns the new status', async () => {
    ...
    invoicesBackend.sendInvoice.mockResolvedValue({ Id: 'invoice-1', Status: 'Sent' });

    const response = await PUT(request, paramsFor('invoice-1'));
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body).toEqual({ id: 'invoice-1', status: 'Sent' });
    expect(invoicesBackend.sendInvoice).toHaveBeenCalledWith('invoice-1', token);
  });
  ...
  it('returns a normalized error when the backend call fails', async () => {
    ...
    invoicesBackend.sendInvoice.mockRejectedValue(new Error('invoice not found'));
