 ...
 *
 * Transitions a Draft invoice to Sent (`Invoice/SendInvoice`, no request
 * body) - backs the "Send Invoice" action on `/invoices/[id]`. Restricted to
...
export async function PUT(_request: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  if (!role || !PROJECT_MANAGEMENT_ROLES.includes(role)) {
    return NextResponse.json(
      { message: 'Only a System Admin or Project Admin can send invoices.' },
      { status: 403 }
    );
  }

  try {
    const dto = await invoicesBackend.sendInvoice(id, accessToken);
    ...
    const details = getBackendErrorDetails(error, 'Could not send the invoice.');
