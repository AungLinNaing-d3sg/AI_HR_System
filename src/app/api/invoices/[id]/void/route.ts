 ...
 *
 * Voids an invoice "regardless of current status" (`Invoice/VoidInvoice`, no
 * request body, per its documented description) - backs the "Void" action on
 * `/invoices/[id]`. Restricted to `PROJECT_MANAGEMENT_ROLES` - see
...
export async function PUT(_request: Request, { params }: RouteParams): Promise<NextResponse> {
  ...
  if (!role || !PROJECT_MANAGEMENT_ROLES.includes(role)) {
    return NextResponse.json(
      { message: 'Only a System Admin or Project Admin can void invoices.' },
      { status: 403 }
    );
  }

  try {
    const dto = await invoicesBackend.voidInvoice(id, accessToken);
    ...
    const details = getBackendErrorDetails(error, 'Could not void the invoice.');
