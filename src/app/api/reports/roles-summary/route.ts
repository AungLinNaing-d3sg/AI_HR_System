...
import { mapUserRolesSummary } from '@/lib/utils/mapReport';
...
import type { UserRolesSummaryResponsePayload } from '@/types/api.types';

/**
 * GET /api/reports/roles-summary?startDate=&endDate=&projectId=
 *
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  const parsed = userRolesSummaryQuerySchema.safeParse(
    pickSearchParams(searchParams, ['startDate', 'endDate', 'projectId'])
  );
  if (!parsed.success) {
    return NextResponse.json(
  ...
  try {
    ...
    logger.error('Generate user roles summary failed', error);
    const details = getBackendErrorDetails(error, 'Could not generate the user roles summary.');
