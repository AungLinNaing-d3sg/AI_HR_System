...
const request = new Request('https://example.com/api/timesheets/periods/period-1/lock', { method: 'PUT' });
...
describe('PUT /api/timesheets/periods/:id/lock', () => {
  beforeEach(() => {
    mockCookieStore.get.mockReset();
    timesheetsBackend.lockTimesheetPeriod.mockReset();
  });
  ...
  it('returns 401 when there is no access token', async () => {
    ...
    expect(timesheetsBackend.lockTimesheetPeriod).not.toHaveBeenCalled();
  });
  ...
  it('locks the period for a ProjectAdmin and returns the confirmation', async () => {
    ...
    timesheetsBackend.lockTimesheetPeriod.mockResolvedValue({ LockedAt: '2026-06-22T01:26:23.930Z' });

    const response = await PUT(request, paramsFor('period-1'));
    const body = await response.json();

    expect(response.status).toBe(200);
    expect(body).toEqual({ id: 'period-1', isLocked: true, lockedAt: '2026-06-22T01:26:23.930Z' });
    expect(timesheetsBackend.lockTimesheetPeriod).toHaveBeenCalledWith('period-1', token);
  });
  ...
  it('returns a normalized error when the backend call fails', async () => {
    ...
    timesheetsBackend.lockTimesheetPeriod.mockRejectedValue(new Error('period not found'));
