...
/**
 * Accessible modal hosting `CountryForm` for both the "+ Add Country" and
 * row-level "Edit" actions on `/admin/countries` (see `CountriesTable`).
 * Mirrors `CurrencyFormModal`'s overlay/`Escape`-to-close conventions, using
 * `role="dialog"` (not `alertdialog`) since this hosts a form rather than a
 * yes/no confirmation prompt.
 */
export function CountryFormModal({ open, mode, country, onSuccess, onClose }: CountryFormModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      ...
      >
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-zinc-500">
              {mode === 'edit' ? 'Update the name of this country.' : 'Add a new supported country.'}
            </p>
          </div>
          <button
            type="button"
        ...
        <CountryForm mode={mode} country={country} onSuccess={onSuccess} onCancel={onClose} />
      </div>
    </div>
  );
}
