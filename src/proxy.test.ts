...
describe('proxy', () => {
  ...
  it('allows a SystemAdmin to reach /admin/currencies', async () => {
    const futureExp = Math.floor(Date.now() / 1000) + 3600;
  ...
  it('allows a SystemAdmin to reach /admin/exchange-rates', async () => {
    const futureExp = Math.floor(Date.now() / 1000) + 3600;
    const token = makeToken({ role: 'SystemAdmin', exp: futureExp });
    const request = new NextRequest('https://example.com/admin/exchange-rates', {
      headers: { cookie: `${ACCESS_TOKEN_COOKIE}=${token}` },
  ...
  it('allows a SystemAdmin to reach /admin/rate-cards', async () => {
    const futureExp = Math.floor(Date.now() / 1000) + 3600;
    const token = makeToken({ role: 'SystemAdmin', exp: futureExp });
    const request = new NextRequest('https://example.com/admin/rate-cards', {
      headers: { cookie: `${ACCESS_TOKEN_COOKIE}=${token}` },
  ...
  it('allows a SystemAdmin to reach /admin/resource-role-types', async () => {
    const futureExp = Math.floor(Date.now() / 1000) + 3600;
    const token = makeToken({ role: 'SystemAdmin', exp: futureExp });
    const request = new NextRequest('https://example.com/admin/resource-role-types', {
      headers: { cookie: `${ACCESS_TOKEN_COOKIE}=${token}` },
